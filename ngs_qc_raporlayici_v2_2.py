...
APP_TITLE = "NGS QC Raporlayıcı 2.2 - Hibrit QC"
...
def sample_group_key(metrics: SampleMetrics) -> str:
    """Ayni hastaya ait DNA/RNA paketlerini analiz kimligiyle birlestir."""
    protocols = analysis_protocol_numbers(metrics.multiqc_analysis_id)
    if protocols:
        return "MP:" + "--".join(f"{number}/{year}" for number, year in protocols)
    return "SAMPLE:" + patient_key(metrics.sample).upper()


def group_display_label(items: list[SampleMetrics]) -> str:
    """Hasta basliginda DNA ham ornek numarasini, yoksa ilk ornegi kullan."""
    dna = next((item for item in items if item.sample_type == "DNA"), None)
    return patient_key((dna or items[0]).sample) if items else "—"
...
class NGSQCApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        info = ttk.Label(
            ...
            text=(
                ...
                "ZIP adı kullanılmaz; örnek kimliği JSON içinden alınır."
            ),
            padding=(12, 0, 12, 8),
        )
        info.pack(fill="x")
    ...
    def analyze(self) -> None:
        ...
        try:
            ...
            status = f"v2.2 | {len(files)} dosya/ZIP incelendi; {len(self.samples)} örnek bulundu; {complete} örnek rapor cümlesi için tam."
            if unparsed:
    ...
    def on_tree_select(self, _event=None) -> None:
        ...
        if selected:
            ...
            for sample_name in selected:
                ...
                if not metrics:
                    continue
                chunks.append(f"Protokol {report_protocol_label(metrics)} – {metrics.sample_type or '?'}\n{metrics.report_sentence()}")
                missing = metrics.missing_required()
                if missing:
