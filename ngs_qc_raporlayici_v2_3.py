...
APP_TITLE = "NGS QC Raporlayıcı 2.3 - Hasta Gruplu Hibrit QC"
...
def report_protocol_label(metrics: SampleMetrics) -> str:
    """DNA/RNA ayri raporlaniyorsa uygun MP protokolunu goster."""
    protocols = analysis_protocol_numbers(metrics.multiqc_analysis_id)
    selected: Optional[tuple[str, str]] = None
    if protocols:
        if metrics.sample_type == "RNA" and len(protocols) >= 2:
            selected = protocols[1]
        else:
            selected = protocols[0]
    if selected:
        number, year = selected
        return f"MP{number}/{year}" if year else f"MP{number}"
    return patient_key(metrics.sample)


def sample_group_key(metrics: SampleMetrics) -> str:
    """Aynı hastanın DNA ve RNA örneklerini tek grupta toplar.

    Yeni dönemde analiz kimliği MP94-26--MP112-26 gibi iki rapor protokolü
    içeriyorsa bu çift hasta anahtarıdır. Eski dönemde veya analiz kimliğinde
    yalnız tek protokol varsa 94D / 94R gibi ham örnek numarası kullanılır.
    """
    protocols = analysis_protocol_numbers(metrics.multiqc_analysis_id)
    if len(protocols) >= 2:
        return "MPPAIR:" + "--".join(f"{number}/{year}" for number, year in protocols)
    return "SAMPLE:" + patient_key(metrics.sample).upper()


def patient_label_for_metrics(metrics: SampleMetrics) -> str:
    """Tabloda ve hasta başlığında gösterilecek ortak hasta numarası."""
    protocols = analysis_protocol_numbers(metrics.multiqc_analysis_id)
    if len(protocols) >= 2:
...
def group_display_label(items: list[SampleMetrics]) -> str:
    """Hasta başlığında DNA'nın hasta numarasını veya çiftin ilk MP numarasını kullan."""
    if not items:
        return "—"
    paired = next(
        (item for item in items if len(analysis_protocol_numbers(item.multiqc_analysis_id)) >= 2),
        None,
    )
    if paired is not None:
        return patient_label_for_metrics(paired)
...
class NGSQCApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        info = ttk.Label(
            ...
            text=(
                ...
                "DNA ve RNA aynı ham numarayla veya MP94/26--MP112/26 gibi ayrı rapor protokolleriyle gelirse aynı hasta altında gruplanır. "
        ...
        widths = {"patient": 70, "sample": 110, "type": 60, "pairs": 115, "q30": 80, "gc": 75, "clean": 105, "retention": 105, "status": 180}
        for col in columns:
    ...
    def analyze(self) -> None:
        ...
        try:
            ...
            status = f"v2.3 | {len(files)} dosya/ZIP incelendi; {len(self.samples)} DNA/RNA örneği bulundu; {complete} örnek QC metni için hazır."
            if unparsed:
    ...
    def export_csv(self) -> None:
        ...
        with open(filename, "w", newline="", encoding="utf-8-sig") as handle:
            ...
            writer.writerow([
                "Hasta", "Rapor protokolü", "Örnek", "Tür", "Okuma çifti (M)", "Toplam bireysel okuma (M)",
