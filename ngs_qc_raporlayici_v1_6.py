...
APP_TITLE = "NGS QC Raporlayıcı 1.6"
...
DNA_OTHER_GENES_NEGATIVE_TEXT = "- Analiz edilen diğer genlerde patolojik mutasyon saptanmamıştır."
...
class NGSQCApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.msi_mss_limit_var = tk.StringVar(value="15")
    ...
    def _build_standard_tab(self, parent: ttk.Frame) -> None:
        ...
        labels_entries = [
            ...
            ("MSI-L alt sınırı:", self.msi_mss_limit_var, 1, 2),
            ("MSI-H sınırı (>):", self.msi_high_limit_var, 2, 0),
        ...
        ttk.Label(
            biomarker_frame,
            text="Varsayılan sınıflama: MSS <%15; MSI-L %15–%40; MSI-H >%40; TMB-H ≥10 mut/Mb. Eşikler değiştirilebilir.",
            foreground="#555555",
    ...
    def rna_results_block(self) -> str:
        ...
        # Seçilen hedefler üstte boş pozitif bulgu alanı olarak bırakılır.
        for label, _genes, placeholder in RNA_FUSION_SELECTIONS:
            if label in selected_groups:
                lines.append(f"- {placeholder}:")

        general_remaining = [gene for gene in RNA_GENERAL_NEGATIVE_GENES if gene not in selected_genes]
        ...
        if not {"NTRK1", "NTRK2", "NTRK3"}.issubset(selected_genes):
            lines.append("- NTRK1, NTRK2 ve NTRK3 genlerinde füzyon izlenmemiştir.")

        if not {"FGFR1", "FGFR2", "FGFR3"}.issubset(selected_genes):
            lines.append("- FGFR-1, FGFR-2 ve FGFR-3 genlerinde füzyon izlenmemiştir.")

        if "ALK" not in selected_genes:
            lines.append("- ALK geninde füzyon izlenmemiştir.")

        # RNA raporunda 'diğer genlerde patolojik mutasyon saptanmamıştır' cümlesi kullanılmaz.
    ...
    def msi_result_text(self) -> str:
        ...
        mss_limit = 15.0 if mss_limit is None else mss_limit
    ...
    def analyze(self) -> None:
        ...
        try:
            ...
            status = f"v1.6 | {len(files)} dosya incelendi; {len(self.samples)} örnek bulundu; {complete} örnek rapor cümlesi için tam."
            if unparsed:
