...
- Yalnizca analiz sonrasi indirilen MultiQC JSON ZIP paketlerini okur.
- ZIP dosya adini kullanmaz; ornek kimligini ZIP icindeki JSON verisinden alir.
- DNA ve RNA icin okuma, Q30 ve GC bilgilerini; DNA icin kapsama, duplikasyon ve FREEMIX metriklerini raporlar.
- Ayni klasorde multiqc_data.zip, multiqc_data (1).zip gibi ayni adli cok sayida paket bulunabilir.
- Standart DNA / RNA rapor metinlerini ve imza duzenini korur.
...
APP_TITLE = "NGS QC Raporlayıcı 2.1 - MultiQC ZIP"
...
class SampleMetrics:
    ...
    def report_sentence(self) -> str:
        ...
        if self.mean_coverage_x is not None:
            alignment_parts.append(f"ortalama kapsama derinliği {tr_num(self.mean_coverage_x, 2)}x")
        if self.median_coverage_x is not None:
            alignment_parts.append(f"medyan kapsama derinliği {tr_num(self.median_coverage_x, 2)}x")
        if self.coverage_100x_pct is not None:
            alignment_parts.append(
                f"en az 100x kapsanan hedef bölge oranı %{tr_num(self.coverage_100x_pct, 2)}"
            )
        elif self.coverage_50x_pct is not None:
            alignment_parts.append(
                f"en az 50x kapsanan hedef bölge oranı %{tr_num(self.coverage_50x_pct, 2)}"
            )
        if alignment_parts:
            if len(alignment_parts) == 1:
                clauses.append(
                    "Post-alignment kalite değerlendirmesinde " + alignment_parts[0] + " olarak bulunmuştur."
                )
            else:
                clauses.append(
                    "Post-alignment kalite değerlendirmesinde "
                    + ", ".join(alignment_parts[:-1])
                    + " ve "
                    + alignment_parts[-1]
                    + " olarak bulunmuştur."
                )
        ...
        if self.freemix_pct is not None:
...
def parse_multiqc_zip(path: Path) -> list[SampleMetrics]:
    ...
    for sample_key, raw_name in raw_by_key.items():
        ...
        # MultiQC ZIP tek basina kullanildiginda temiz veri miktarini, filtreyi gecen
        # bireysel read sayisi ile R1/R2 ortalama okuma uzunlugundan hesapla.
        lengths = []
        for suffix in ("_1", "_2"):
            value = _metric_sample_value(metric_values, "fastqc-avg_sequence_length", raw_name + suffix)
...
class NGSQCApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        toolbar.pack(fill="x")
        ttk.Button(toolbar, text="MultiQC ZIP Seç", command=self.choose_files).pack(side="left", padx=(0, 6))
        ...
        info = ttk.Label(
            self,
            text=(
                "Yalnızca analiz sonrası indirilen MultiQC JSON ZIP paketleri kullanılır. ZIP adı kullanılmaz; "
                "örnek kimliği ZIP içindeki JSON verisinden alınır. Aynı klasörde adı aynı veya Windows tarafından "
                "numaralandırılmış çok sayıda ZIP bulunabilir."
            ),
            padding=(12, 0, 12, 8),
        )
        info.pack(fill="x")
    ...
    def choose_files(self) -> None:
        filenames = filedialog.askopenfilenames(
            title="MultiQC JSON ZIP paketlerini seçin",
            filetypes=[
                ("MultiQC ZIP paketleri", "*.zip"),
                ("Tüm dosyalar", "*.*"),
            ],
        )
        if filenames:
    ...
    def choose_folder(self) -> None:
        folder = filedialog.askdirectory(title="MultiQC ZIP paketlerinin bulunduğu klasörü seçin")
        if folder:
    ...
    def collect_files(self) -> tuple[list[Path], list[str]]:
        ...
        for selected in self.selected_paths:
            if not selected.exists():
                ...
                continue
            if selected.is_dir():
                for candidate in selected.rglob("*.zip"):
                    if is_multiqc_zip(candidate):
                        files.append(candidate)
                continue
            if selected.suffix.lower() == ".zip" and is_multiqc_zip(selected):
                files.append(selected)
            else:
                warnings.append(f"MultiQC JSON ZIP paketi değil, atlandı: {selected.name}")
        return list(dict.fromkeys(files)), warnings

    def analyze(self) -> None:
        if not self.selected_paths:
            messagebox.showinfo(APP_TITLE, "Önce MultiQC ZIP dosyalarını veya bunların bulunduğu klasörü seçin.")
            return
        try:
            ...
            status = f"v2.1 | {len(files)} MultiQC ZIP incelendi; {len(self.samples)} örnek bulundu; {complete} örnek rapor cümlesi için tam."
            if unparsed:
